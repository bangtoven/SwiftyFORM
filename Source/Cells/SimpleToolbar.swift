...
public class SimpleToolbar: UIToolbar {
	...
	public lazy var previousButton: UIBarButtonItem = {
		let image = UIImage(named: "SwiftFORMArrowLeft", in: Bundle(for: self.dynamicType), compatibleWith: nil)
		if let image = image {
	...
	public lazy var nextButton: UIBarButtonItem = {
		let image = UIImage(named: "SwiftFORMArrowRight", in: Bundle(for: self.dynamicType), compatibleWith: nil)
		if let image = image {
			let image2 = image.withRenderingMode(.alwaysTemplate)
